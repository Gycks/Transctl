...
class TMStore:
    ...
    def upsert(self, session: Session, lang: str, hash_: str, translation: str):
        """
    ...
    def prune(self, session: Session, policy: PrunePolicy):
        """
        ...
        if policy.vacuum:
            session.execute("PRAGMA incremental_vacuum;")
            session.commit()
